...
<script type="text/javascript">
  var ctx = document.getElementById("chartGangguanJaringan").getContext('2d');
  <?php
  $gangguan_label = array();
  $gangguan_value = array();
  if (!empty($dashboard_gangguan_jaringan_per_bulan)) {
    foreach ($dashboard_gangguan_jaringan_per_bulan->result_array() as $row) {
      array_push($gangguan_label, $row['periode_bulan']);
      array_push($gangguan_value, $row['jumlah']);
    }
  }; ?>
  var myChartGangguan = new Chart(ctx, {
    type: 'pie',
    data: {
      labels: <?php echo json_encode($gangguan_label); ?>,
      datasets: [{
        data: <?php echo json_encode($gangguan_value); ?>,
        backgroundColor: [
          'rgba(255,99,132,1)',
          'rgba(54, 162, 235, 1)',
          'rgba(255, 206, 86, 1)',
  ...
  function getUpdate(like) {
    $.ajax({
      url: "<?php echo base_url(); ?>admin/ajax_dashboard_gangguan_jaringan_per_bulan_filter",
      method: "POST",
      data: {
        wilayah: like
      },
      ...
      success: function(data) {
        ...
        data.forEach(item => {
          dataSetNew.push(item.jumlah)
          labelNew.push(item.periode_bulan)
        })

        myChartGangguan.data.datasets.forEach((dataset) => {
          dataset.data = dataSetNew
        })
        myChartGangguan.labels = labelNew
        myChartGangguan.update()
      }
    });
  }

  $('#filter_asman_chart_gangguan').on('change', function(e) {
    var wilayah = e.target.value;
    getUpdate(wilayah);
  })
</script>
